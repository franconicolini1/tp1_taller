...
pub fn run_program() -> io::Result<()> {
    let stdin = io::stdin();

    let mut buffer1 = String::new();
    stdin.read_line(&mut buffer1)?;
    if buffer1.ends_with('\n') {
        buffer1 = buffer1[0..buffer1.len() - 1].to_string();
    }

    let vector1 = read_file_lines(buffer1); // The path is src/example1.txt
    if vector1.is_err() {
    ...
    if buffer2.ends_with('\n') {
        buffer2 = buffer2[0..buffer2.len() - 1].to_string();
    }

    let vector2 = read_file_lines(buffer2); // The path is src/example2.txt
    if vector2.is_err() {
...
fn read_file_lines(path: String) -> Result<Vec<String>, std::io::Error> {
